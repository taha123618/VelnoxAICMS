...
The authenticated user is available in all pages via `useAuth()` composable:

```ts
import { useAuth } from '@modules/Auth/resources/composables/use-auth';
const user = useAuth(); // ComputedRef<Modules.Auth.Data.AuthenticatedUserData>
```

The `AuthenticatedUserData` type includes:
- `user.value.isAdmin` — `true` when the user has the `admin` Spatie role
- `user.value.can` — `AuthorizationData` with per-permission booleans

Use `user.value.isAdmin` for admin-only UI elements (e.g. Horizon sidebar link).

### TypeScript Type Regeneration

After adding or changing a PHP `#[TypeScript]` DTO (e.g. `AuthenticatedUserData`, `AuthorizationData`), regenerate the TypeScript definitions:

```bash
php artisan typescript:transform
```

This updates `resources/js/types/generated.d.ts`. Always run this after DTO changes or the frontend will have stale/missing type errors.

### Role-Gated Navigation Items

Conditionally include nav items based on `user.value.isAdmin` using spread syntax in `computed`:
        ...
        children: [
            {
                label: 'Horizon',
                icon: 'ph:queue',
                // Use window.location for non-Inertia/non-Ziggy URLs
...
> Use `window.location.href` (not `router.visit`) for external URLs like `/horizon` that are outside the Inertia SPA.
